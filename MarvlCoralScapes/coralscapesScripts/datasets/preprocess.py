...
#     # Optional different windowing for labels (unchanged if using same stride/size)
#     if window_size_label is not None and stride_label is not None:
#         height, width = labels.shape[-2:]
#         label_windows = []

#         y_positions = list(range(0, height - window_size_label + 1, stride_label))
#         if y_positions[-1] + window_size_label < height:
#             y_positions.append(height - window_size_label)

#         x_positions = list(range(0, width - window_size_label + 1, stride_label))
#         if x_positions[-1] + window_size_label < width:
#             x_positions.append(width - window_size_label)
...
#     # Stack windowed results
#     input_windows = torch.stack([torch.stack(row, dim=0) for row in input_windows], dim=0)
#     label_windows = torch.stack([torch.stack(row, dim=0) for row in label_windows], dim=0)
#     return input_windows, label_windows


# def stitch_predictions(voutputs, window_size, stride, height, width):
#     """
#     Reconstruct full-size prediction from overlapping sliding window outputs.
    
#     Args:
#         voutputs (Tensor): [n_y, n_x, B, C, win, win]
...
#     Returns:
#         Tensor: [B, C, H, W] — full reconstructed image
#     """
#     n_y, n_x, B, C, _, _ = voutputs.shape
#     device = voutputs.device

#     full_output = torch.zeros((B, C, height, width), device=device)
#     count_map = torch.zeros((B, 1, height, width), device=device)

#     for i in range(n_y):
#         for j in range(n_x):
#             y = i * stride
#             x = j * stride
#             window = voutputs[i, j]  # [B, C, win, win]

#             h_crop = min(window_size, height - y)
#             w_crop = min(window_size, width - x)

#             full_output[:, :, y:y+h_crop, x:x+w_crop] += window[:, :, :h_crop, :w_crop]
#             count_map[:, :, y:y+h_crop, x:x+w_crop] += 1

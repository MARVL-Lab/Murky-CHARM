...
def get_batch_predictions(data, model, device, loss_fn = None):
    ...
    # Attempt to use custom loss function (Added by yx)
    # if(loss_fn):
    #     # print(outputs.logits.to(device))
    #     # print(data["labels"].to(device))
    ...
    #     target_sizes = [
    #         (image.shape[0], image.shape[1]) for image in data["labels"]
    #     ]
...
class Benchmark_Run:
    ...
    def __init__(self, run_name: str, model_name: str, N_classes: int, 
        ...
        if(model_name == "deeplabv3+resnet50"):
            ...
            pretrained_model_name="EPFL-ECEO/segformer-b5-finetuned-coralscapes-1024-1024"
            # pretrained_model_name = "EPFL-ECEO/segformer-b5-finetuned-coralscapes-1024-1024_20"
            if is_coralscape:
                N_classes = 40 # Ensure it loads the right finetuned model. Added by yx
            self.model = SegformerForSemanticSegmentation.from_pretrained(
                pretrained_model_name, id2label={i:i for i in range(0, N_classes)}, 
                semantic_loss_ignore_index = 255, ignore_mismatched_sizes=True # FIXME we 
            )
            print(f"Using the model {pretrained_model_name}.")
        ...
        #         checkpoint = load_safetensors(model_checkpoint)
        ...
        if(training_hyperparameters.loss):
            ...
            print(loss)
            # print(training_hyperparameters.loss["weight"])
    ...
    def validation_epoch(self, val_loader):
        ...
        return running_loss / (i+1)   
        # return running_loss / valid_batches if valid_batches > 0 else 0.0 
...
# Added by yx
def convert_labels(inference_prediction):
...
def predict(inputs, benchmark_run, output_size = None, window_dims = None, is_coralscape = False):
    """
    ...
    # here
    if is_coralscape:
        outputs = convert_labels(outputs) # added by yx
